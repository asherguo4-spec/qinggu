...
1. 账号与服务
您需要注册并登录账号使用我们的核心生成服务。本服务提供AI首饰与手办设计技术等各类功能。

2. 用户行为规范
您不得利用本服务制作、分享任何违反国家法律法规、侵犯他人知识产权的内容。对于违规内容，平台有权采取封号或删除措施。

3. 知识产权保护
请保证您所上传的图片及描述内容不侵犯第三方的版权、商标及其他合法权益。由此引发的纠纷由您自行承担。使用本应用生成结果的一切合法权益与应用无关。

4. 责任免除
平台基于尖端AI生成系统，由于技术的不可遇风险可能导致未达预期，一切以此为准。` 
    },
    privacy: { 
      title: '隐私政策', 
      content: `为了您的产品体验与权益保障，我们的合规政策说明如下：

1. 信息收集范围
在您授权或使用生成等基本服务时，我们仅在最小原则下收集您的必要信息，如设备类型、基础日志等以保障产品正常运行。

2. 数据隐私与保护
我们严格遵照法律与行业标准对您的信息进行加密防护。您上传的参考图像等数据仅用于本地任务分析计算与生成，并进行严密防护保存，绝不会出售或泄露给任何形式的第三方。

3. 您的权利
您对个人信息具有删除与随时退出的权利。对于数据安全或者撤销相关授权，您可联系官方获得解决。` 
    },
    refund: { 
      title: '退款政策', 
      content: `关于我们的消费相关的退款政策说明：

1. 关于算力消耗型服务（如生图与AI模型）
当用户的余额开始消耗由于已经产生了算力占用、API成本等，此类虚拟商品一旦提供或使用将无法恢复，因此非特殊系统错误不支持无理由退还余额操作。

2. 实体订单（手办/造物等）
定制工艺与打印加工，一经进入排单与生产流程即消耗材料与人工，非严重产品质量缺陷不支持退款退货。若是运输受损，请在签收内要求补偿处理。

具体售后与争议可联系您专属的订单服务渠道。` 
    }
  };
